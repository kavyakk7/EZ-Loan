...
try
	{
		...
		String query = "insert into personal_loan(BankName,Rate,EMI,ProcessingFee,MinLoanAmount,MaxLoanAmount,Link) values('"+bank+"' ,'"+Double.parseDouble(rate)+"','"+Double.parseDouble(emi)+"','" +Double.parseDouble(pfee)+ "','" +Double.parseDouble(minloan)+ "','" +Double.parseDouble(maxloan)+ "','" +link+ "')";
		int i = st2.executeUpdate(query);
		if(i>0)
		{
			out.println("<script type=\"text/javascript\">");
		      out.println("alert('Details added!! ');");
		      out.println("location='AddPersonalLoanPage.jsp';");
		      out.println("</script>");
	  	}
		else
		{
